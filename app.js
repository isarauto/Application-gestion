...
async function saveRentalFromForm() {
  ...
  if (s && e) {
    ...
    if (ed < sd) { alert('تاريخ الانتهاء يجب أن يكون بعد تاريخ البدء'); return; }
  }
  ...
  if (editingRentalId) {
    ...
    Object.assign(r, { customerName: customer, carModel, carNumber, startDate: s, endDate: e, days, pricePerDay: price, totalAmount: total, taxAmount: tax, netAmount: net, paymentMethod });
    ...
    const r = { id: genId('RENT'), customerName: customer, carModel, carNumber, startDate: s, endDate: e, days, pricePerDay: price, totalAmount: total, taxAmount: tax, netAmount: net, paymentMethod, isPaid: false };
...
function updateRentalsUI() {
  ...
  appData.rentals.slice().reverse().forEach(r => {
    const tr = document.createElement('tr');
    [r.customerName, r.carModel, r.carNumber, formatDate(r.startDate), (r.endDate ? formatDate(r.endDate) : '-'), `${formatNumberLatn(r.days)} يوم`, formatCurrency(r.netAmount)]
