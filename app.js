...
async function saveRentalFromForm() {
  ...
  if (editingRentalId) {
    ...
    Object.assign(r, { customerName: customer, clientId, carModel, carNumber, startDate: s, endDate: e, startTime: sTime, endTime: eTime, days, pricePerDay: price, totalAmount: total, taxAmount: tax, netAmount: net, paymentMethod });
    ...
    const r = { id: genId('RENT'), customerName: customer, clientId, carModel, carNumber, startDate: s, endDate: e, startTime: sTime, endTime: eTime, days, pricePerDay: price, totalAmount: total, taxAmount: tax, netAmount: net, paymentMethod, isPaid: false };
...
function computeClientStats(client) {
  ...
  const totalSpent = list.reduce((sum, r) => sum + (r.netAmount || 0), 0);
  let last = '';
  list.forEach(r => { const d = r.startDate || ''; if (d && d > last) last = d; });
